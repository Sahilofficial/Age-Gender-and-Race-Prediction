...
dataset_dir = r'C:\Users\Lenovo\jupyter project\age and gender\UTKFace'
...
# #### Lets define a function for extracting the data from our dataset. This function will be used to iterate over each file of the UTK dataset and return a Pandas Dataframe containing all the fields (age, gender and sex) of our records.
...
# ### Data visualization
# 
# > Generate plots based on a given Pandas series:
...
# ##### It is clearly seen that our dataset is mostly composed of individuals which age varies between 20 and 30 years, followed by individuals ranging from 30-40 years and then 40-60 years old. These groups represent around 70% of our dataset, so we can believe that we are going to have a good accuracy on predicting individuals in these ranges.
...
# ##### Now let's train our model with a batch size of 40 for both valid and train sets. We will be using a ModelCheckpoint callback in order to save the model on disk at the end of each epoch.
# 
# 
...
# #### Finally, let's print the classification reports for each feature on the test set.
# 
